...
- Host commands: `CMD_SET_HM` (head-moving mode), `CMD_SET_FOCUS` (target cluster), `CMD_SET_BIO_MS`/`CMD_SET_TARGETS_MS` (telemetry cadence), `CMD_PING`, `CMD_SET_GUARD_RAILS` (vitals BR/HR acceptance ranges, 4x u16 centi-bpm), `CMD_RESET` (re-init mmWave radar, ACK/ERR reply)
